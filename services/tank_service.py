...
def apply_sensor_spike_filter(tank_state, new_distance_cm, threshold_cm, max_consecutive):
    ...
    Behaviour:
      ...
      - Otherwise require ``max_consecutive`` mutually consistent readings
        around the new value before accepting it as the new baseline.
    ...
    last_raw = tank_state.get("last_raw_distance_cm")
    try:
        candidate_is_consistent = (
            last_raw is not None
            and abs(float(new_distance_cm) - float(last_raw)) <= threshold_cm
        )
    except (TypeError, ValueError):
        candidate_is_consistent = False

    count = (
        int(tank_state.get("consecutive_spike_count", 0) or 0) + 1
        if candidate_is_consistent
        else 1
    )

    # A real level change remains stable; an isolated bad echo does not. Once
    # the candidate is confirmed, adopt it instead of locking in "unknown".
    if count >= max(1, int(max_consecutive)):
        return {"status": "accepted", "count": 0}
